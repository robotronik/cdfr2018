...
#define LL_DMA_STREAM_x(x) CAT(LL_DMA_STREAM_, x)
...
#define USARTx_IRQHandler(usart_x, dma_x, dma_stream_x, iface)		\
  ...
  if(LL_USART_IsActiveFlag_IDLE(USARTx(usart_x))){				\
    ...
    uint16_t next_pos = RECEIVED_DATA(dma_x, dma_stream_x);		\
									\
...
#define FLAG_END(flag, dma_x, dma_stream_x) CAT(flag, dma_stream_x)(CAT(DMA, dma_x))
#define LL_DMA_IsActiveFlag_xx(flag, dma_x, dma_stream_x) CAT(LL_DMA_IsActiveFlag_, FLAG_END(flag, dma_x, dma_stream_x))
#define LL_DMA_ClearFlag_xx(flag, dma_x, dma_stream_x) CAT(LL_DMA_ClearFlag_, FLAG_END(flag, dma_x, dma_stream_x))

#define DMAx_Streamx_IRQHandler(dma_x, dma_stream_x, iface)	\
  uint16_t next_pos;						\
  /* Transfert Complete, ie. buffer loop complete */		\
  if(LL_DMA_IsActiveFlag_xx(TC, dma_x, dma_stream_x)){		\
    /* Acquit interrupt */					\
    LL_DMA_ClearFlag_xx(TC, dma_x, dma_stream_x);		\
    								\
  ...
  else if(LL_DMA_IsActiveFlag_xx(HT, dma_x, dma_stream_x)){		\
  /*									\
   ...
   */									\
									\
    /* Acquit interrupt */						\
    LL_DMA_ClearFlag_xx(HT, dma_x, dma_stream_x);			\
									\
    /* Number of bytes received since the last Transfert Complete event */ \
    next_pos = RECEIVED_DATA(dma_x, dma_stream_x);			\
									\
